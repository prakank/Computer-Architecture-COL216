...
area_mod:
    ...
    sub.s $f2,$f2,$f4  # for x1-x2
    sub.s $f4,$f5,$f3  # for y2-y1

    c.eq.s $f4,$f7  # checking if denom=0
    bc1t D_

    mul.s $f3,$f3,$f3
    mul.s $f5,$f5,$f5
    add.s $f3,$f3,$f5  # computing y1^2 + y2^2
    mul.s $f2,$f2,$f3  # computing (x1-x2)*(y1^2+y2^2)
    div.s $f2,$f2,$f4  # computing (x1-x2)*(y1^2+y2^2)/((y2-y1))  
    div.s $f2,$f2,$f6  # computing (x1-x2)*(y1^2+y2^2)/((y2-y1)*2)  
    abs.s $f2,$f2
    add.s $f1,$f1,$f2
